#!/bin/bash
# Safebox system-protocol-api — broker host-side setup
#
# NOTE: this does NOT install the Safebox components
# (base/system/dnsclient/autohost). It ONLY sets up the host-side
# prerequisites the system-protocol-api CONTAINER needs before docker-compose
# brings it up: the safebox-api user + safebox-hmac group, the HMAC key at
# /etc/safebox/system-api.key, managed-containers.json, and the
# system-registry.json M-of-N governance registry.
#
# Run it AFTER the component installers and BEFORE `docker compose up`, as
# provision-safebox-ami.sh does. See docs/AWS-NITRO-SETUP.md for the full order.
