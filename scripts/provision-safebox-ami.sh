...
configure_zfs() {
    ...
    ssh -o StrictHostKeyChecking=no "ubuntu@$PUBLIC_IP" << 'REMOTE_SCRIPT'
        ...
        # Create ZFS pool. MUST be named 'safebox-pool' — install-base.sh
        # hard-fails if the pool has any other name, and creates all datasets
        # (safebox, docker, mariadb, tenants) under it. Flags match the
        # canonical form documented in install-base.sh:
        #   ashift=12       align to 4K sectors (EBS)
        #   compression=lz4 compress-before-encrypt (ZFS is the encryption layer)
        #   atime=off       no per-read metadata writes
        sudo zpool create -o ashift=12 -O compression=lz4 -O atime=off \
            safebox-pool /dev/nvme1n1
        sudo zpool status

        # Datasets are created by install-base.sh (safebox, docker, mariadb,
        # tenants) with per-dataset tuning. Do NOT pre-create them here — the
        # installer owns that layout, and a stray dataset would diverge from
        # what the MariaDB/ZFS tuning expects.

        echo "✓ ZFS pool 'safebox-pool' created (datasets created by install-base.sh)"
REMOTE_SCRIPT
...
install_infrastructure() {
    ...
    ssh -o StrictHostKeyChecking=no "ubuntu@$PUBLIC_IP" << 'REMOTE_SCRIPT'
        ...
        sudo rm -rf /opt/safebox-infrastructure
        unzip -q Infrastructure.zip -d /opt/safebox-infrastructure
        cd /opt/safebox-infrastructure

        # ── Preflight ────────────────────────────────────────────────────────
        # Fail fast if the box isn't actually ready (ZFS pool, keys, Nitro).
        # Turns a half-installed mystery box into a clear error at second zero.
        if [ -f scripts/preflight.sh ]; then
            sudo bash scripts/preflight.sh
        fi

        # ── Full component install, in dependency order ──────────────────────
        # NOTE (reconciliation): the AMI path previously ran only
        # scripts/install.sh, which installs ONLY the system-protocol-api
        # docker broker — it skipped base (ZFS/MariaDB/docker daemon), system,
        # dnsclient, and autohost entirely. That produced a box with the broker
        # and nothing else. The correct AMI install is the full sequence below.
        # base MUST run first: it verifies the 'safebox-pool' ZFS pool exists,
        # creates the datasets, tunes MariaDB, and configures the docker daemon
        # (zfs storage driver) that everything else depends on.
        echo "▶ [1/4] base (ZFS datasets, MariaDB tuning, docker daemon)..."
        sudo bash aws/scripts/components/base/install-base.sh \
            aws/scripts/components/base

        echo "▶ [2/4] system (privileged broker + embeddings subprocess)..."
        sudo bash aws/scripts/components/system/install-system.sh \
            aws/scripts/components/system

        echo "▶ [3/4] dnsclient (IP announcement to control plane)..."
        sudo bash aws/scripts/components/dnsclient/install-dnsclient.sh \
            aws/scripts/components/dnsclient

        # autohost provisioner code comes from the vendor/autohost submodule.
        # The zip must already contain the RESOLVED submodule (git submodule
        # update --init before packaging); install-autohost.sh hard-fails if
        # vendor/autohost/src/autohost.js is absent.
        echo "▶ [4/4] autohost (custom-domain provisioning, vendored Autohost)..."
        sudo bash aws/scripts/components/autohost/install-autohost.sh \
            aws/scripts/components/autohost

        # ── Broker host-side prerequisites (for the system-protocol-api container) ──
        # The docker-compose stack runs system-protocol-api as a container that
        # mounts /etc/safebox and needs the safebox-api user, the HMAC key at
        # /etc/safebox/system-api.key, and the system-registry.json M-of-N
        # registry to exist on the host FIRST. scripts/install-system-api-host.sh sets these up.
        # (It does NOT install the components above — despite its generic name it
        # only does broker host-side setup. Run it here, after the components,
        # before bringing up the stack.)
        echo "▶ broker host-side setup (safebox-api user, HMAC key, M-of-N registry)..."
        sudo bash scripts/install-system-api-host.sh

        # ── Bring up the docker-compose stack ────────────────────────────────
        # The system-protocol-api container (and the rest of the runtime) is
        # orchestrated by docker/docker-compose.yml. base installed the docker
        # daemon; the broker prerequisites are now in place; start the stack.
        echo "▶ Starting docker-compose stack..."
        cd /opt/safebox-infrastructure/docker
        sudo docker compose up -d || sudo docker-compose up -d

        echo "✓ Infrastructure installed (base + system + dnsclient + autohost + broker + stack)"
REMOTE_SCRIPT
