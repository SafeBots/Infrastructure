...
> **STATUS (2026-07-09): read this first.** This document mixes what is BUILT
> with what is DESIGNED-BUT-UNBUILT, and the two were previously indistinguishable.
> Correcting that here:
>
> - **BUILT and shipping:** `FLUSH TABLES WITH READ LOCK` → clean ZFS snapshot,
>   via `/opt/safebox/bin/flush-and-snapshot.sh`. Cross-Safebox replication is
>   **`zfs send | ssh | zfs receive`** — block-level, already-compressed,
>   already-encrypted deltas against a snapshot. This is the real, working path.
>   It is NOT borg, and it is NOT rsync.
> - **DESIGNED but NOT built:** `borg-chunk.py` and `backup-safebox.sh` below.
>   These describe a *borg-like* (a from-scratch reimplementation of borg's
>   content-defined chunking technique — NOT a dependency on BorgBackup) layer
>   for sub-block, position-independent dedup across machines. **No such file
>   exists in the repo.** The "600+ lines added" claim was aspirational. If we
>   want this layer we must build it; until then `zfs send` covers replication.
>
> Why `zfs send` is enough for now: ZFS already knows exactly which blocks
> changed since the last snapshot, so an incremental send ships only the delta —
> the dedup benefit borg-style chunking would add is marginal over ZFS's own
> block-delta send, and it comes at the cost of a whole custom chunk store to
> maintain. Content-defined chunking wins mainly when data *shifts position*
> (insertions that slide everything down), which ZFS's fixed-block send handles
> less efficiently. That is a real but narrow win; revisit if it becomes the
> bottleneck.

## What Changed

Added complete support for **FLUSH TABLES WITH READ LOCK** method for consistent file-level snapshots. In the shipped implementation this feeds `zfs snapshot` + `zfs send` (see the status banner above); the borg-like chunker described further down remains a design, not code.
