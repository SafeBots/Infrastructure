...
> **STATUS (2026-07-09): read this first.** This document's title and framing are
> STALE and do not describe what the system implements. Correcting that here:
>
> - **BUILT and shipping:** the backup/consistency path is a `FLUSH TABLES WITH
>   READ LOCK` around an atomic **ZFS snapshot**, via
>   `/opt/safebox/bin/flush-and-snapshot.sh` (source:
>   `aws/scripts/components/base/helpers/flush-and-snapshot.sh`), governed through
>   the `zfs-snapshot` action in `opsSystem.js`. Cross-Safebox replication is
>   **`zfs send | ssh | zfs receive`** — block-level, already-compressed,
>   already-encrypted snapshot deltas.
> - **NOT used:** neither **XtraBackup**/`mariabackup` nor **borg** is installed
>   or invoked by any script in this repo. The "XtraBackup Primary" title and the
>   borg references below are aspirational design discussion from an earlier
>   iteration. No XtraBackup or borg pipeline exists.
> - **Why ZFS snapshot + send is the real design:** the MariaDB dataset lives on
>   ZFS, so a snapshot is atomic and near-instant, and `zfs send` ships only the
>   changed blocks against the prior snapshot. That subsumes what XtraBackup's
>   incremental mode and borg's dedup would each provide, at the filesystem layer,
>   without an application-level backup tool. See `FLUSH-TABLES-UPDATE.md` for the
>   companion note on why FLUSH TABLES is still wrapped around the snapshot.
>
> The technical discussion below (InnoDB crash-consistency, when FLUSH TABLES is
> or isn't needed) is still accurate as *background*; only the "we use XtraBackup
> / borg" framing is wrong. Treat this file as design history, not as the
