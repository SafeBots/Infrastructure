...
**Wired by `install-base.sh`** — the dataset tuning and `/etc/my.cnf.d/safebox.cnf` below are written at build time, not left to defaults.

**File-per-table + ZFS-safe flushing (`/etc/my.cnf.d/safebox.cnf`):**
```ini
[mysqld]
datadir = /srv/mariadb/data

# One .ibd per table — required for per-table ZFS dataset isolation
innodb_file_per_table = 1

# InnoDB page size == ZFS recordsize on safebox-pool/mariadb
innodb_page_size = 16k

# O_DIRECT: InnoDB's buffer pool owns data caching; don't also cache in ARC
innodb_flush_method = O_DIRECT

# Clean (not just crash-consistent) state for FLUSH TABLES snapshots
innodb_flush_log_at_trx_commit = 1
sync_binlog = 1

# ZFS is copy-on-write + checksummed; InnoDB doublewrite is redundant on ZFS
innodb_doublewrite = 0

max_connections   = 500
thread_cache_size = 50
# innodb_buffer_pool_size = 16G   # size per instance, leave ARC headroom
```

**ZFS dataset tuning (set on `safebox-pool/mariadb`, inherited by children):**
```bash
zfs set recordsize=16k        safebox-pool/mariadb  # 1 InnoDB page = 1 ZFS record
zfs set primarycache=metadata safebox-pool/mariadb  # no double-buffering vs InnoDB pool
zfs set logbias=throughput    safebox-pool/mariadb  # large sequential InnoDB writes
```

**Why these three matter (the double-buffering trap):** without `recordsize=16k`, every 16 KiB InnoDB write becomes a 128 KiB read-modify-write (8× amplification). Without `primarycache=metadata`, the same data pages get cached twice — once in InnoDB's buffer pool, once in the ZFS ARC — halving effective RAM. Without `innodb_doublewrite=0`, InnoDB writes every page twice to guard against torn pages that ZFS's copy-on-write makes impossible anyway. These are the settings that separate "works in dev" from "doesn't waste half the box in production."

**ZFS dataset structure:**
```
safebox-pool/mariadb (recordsize=16k, primarycache=metadata, inherited by all children)
...
**Clean cross-Safebox snapshots (`/opt/safebox/bin/flush-and-snapshot.sh`):** the governed `zfs-snapshot` action gives a crash-consistent image (InnoDB recovers on restore). For the ship-to-another-Safebox path (`zfs send | ssh | zfs receive`, used instead of SQL replication) the helper wraps the snapshot in `FLUSH TABLES WITH READ LOCK` — held across the atomic snapshot in a single session — so the far side needs no recovery. Lock window is typically 1-5 seconds (the flush, not the snapshot).
