...
### System Component Sockets

The system component (the Safebox-facing host API) listens on **Unix domain sockets only — no TCP port**. There is one socket per managed container plus one control socket:

```
/run/safebox/
├── control.sock                          ← host-only; controls _host scope
└── containers/
    ├── safebox-app-foo.sock              ← bind-mount target for foo only
    ├── safebox-app-bar.sock              ← bind-mount target for bar only
    └── ... (one per managed container)
```

Each per-container socket is bind-mounted into exactly one Docker container at `/run/safebox/system.sock`. The container's Safebox Node connects to that conventional path. The system component, knowing which socket received the connection, knows which container is calling — without trusting the request body. This is the **kernel-level identity guarantee**: container foo has no path in its mount namespace through which to reach bar's socket.

The control socket is not bind-mounted into any container. It's reachable only from host-side processes (mode 0660, owner `safebox-infra`). It carries host-scope operations: `dnf`, `/models/*`, `/containers/*` (lifecycle).

| Socket | Reachable from | HMAC key | Permitted endpoints |
|--------|---------------|----------|---------------------|
| `/run/safebox/control.sock` | Host (safebox-infra) only | Master | `dnf` via `/system`; all of `/models/*`; all of `/containers/*` |
| `/run/safebox/containers/<name>.sock` | Bind-mounted into one container | Per-container, HKDF-derived | `npm/pip/composer/cargo/gem/git/migrate/zfs-*` via `/system`; all of `/test/*` |

Per-container HMAC keys are derived deterministically from the master via HKDF with `info = "safebox-container-<name>-v1"`, so foo's key and bar's key are cryptographically independent. The container's key file is bind-mounted read-only into the container at `/etc/safebox/system.hmac`.

For the full transport protocol, signing format, error codes, and integration examples, see [`Safebox.md`](Safebox.md).
