...
> **STATUS (2026-07-09): design history — NOT the implemented backup strategy.**
> This document describes an XtraBackup + content-defined-chunking ("borg-like"
> / Prolly-tree) pipeline that was explored but **never built**. Specifically:
>
> - `/srv/safebox/bin/borg-chunk.py`, `chunk-xtrabackup-stream.py`, and the
>   `xtrabackup --backup` invocations below **do not exist in the repo** and are
>   not installed or called by any script.
> - The **actually implemented** backup/consistency path is `FLUSH TABLES WITH
>   READ LOCK` around an atomic **ZFS snapshot** (`flush-and-snapshot.sh`,
>   governed via the `zfs-snapshot` action), with cross-Safebox replication by
>   **`zfs send | ssh | zfs receive`**. ZFS's own block-level incremental send
>   provides the dedup/delta benefit this document was reaching for, at the
>   filesystem layer, with no application-level backup tool.
>
> Keep this file as a record of the design exploration. For the real procedure,
> see the status banner in `XTRABACKUP-PRIMARY.md` and `FLUSH-TABLES-UPDATE.md`.
