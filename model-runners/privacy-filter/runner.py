...
import uvicorn
# torch and transformers are imported lazily inside PrivacyFilterModel.load()
# so the module can be imported (for tests, capability checks, HMAC verification)
# on hosts without the ML stack installed — matching the other runners.
...
class PrivacyFilterModel:
    ...
    def load(self):
        ...
        # Lazy imports — keep torch/transformers out of module import so the
        # runner can be imported without the ML stack (tests, HMAC, capabilities).
        import torch
    ...
    def redact(self, text: str, entities: Optional[List[str]] = None, mode: str = "redact"):
        ...
        import torch  # available because load() succeeded before redact() runs
