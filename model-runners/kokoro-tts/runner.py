"""
Kokoro TTS Runner — Safebox Local Service v1.0

Wraps the Kokoro TTS library (StyleTTS2-derived, 82M params, Apache 2.0)
...
Streaming:
  ...
  Kokoro produces them. Useful for long-form text where the UI wants to
...
# ─── Configuration ─────────────────────────────────────────────────────
RUNNER_ID  = os.getenv("RUNNER_ID",  "safebox-kokoro-1")
...
MODEL_NAME       = os.getenv("MODEL_NAME",       "kokoro-82m")
KOKORO_LANG_CODE = os.getenv("KOKORO_LANG_CODE", "a")    # 'a' = American English; see Kokoro docs
...
WORK_DIR = Path(os.getenv("KOKORO_WORK_DIR", "/data/output"))
...
CAP_EMOTION       = os.getenv("CAP_EMOTION",       "false").lower() == "true"  # Kokoro doesn't expose emotion control
CAP_STREAMING     = True
...
# ─── App ───────────────────────────────────────────────────────────────
app = FastAPI(title="Kokoro Safebox Runner", version="1.0")
...
# ─── Kokoro wrapper ────────────────────────────────────────────────────
class KokoroRunner:
    """Lazily import Kokoro. The library loads model weights and voice
    library at first construction; takes 5-15 seconds depending on hardware."""
    def __init__(self):
        self.ready  = False
        self.pipeline = None
    ...
    def load(self):
        if self.ready:
            return
        try:
            # Kokoro's main pipeline class. Newer versions of the library
            # may use a different import path; fall back to the common shape.
            try:
                from kokoro import KPipeline  # type: ignore
            except ImportError:
                from kokoro_tts import KPipeline  # type: ignore  # alternate name
            logger.info(f"Loading Kokoro with lang_code={KOKORO_LANG_CODE}")
            self.pipeline = KPipeline(lang_code=KOKORO_LANG_CODE)
            # Discover voice library
            try:
                # Newer Kokoro versions expose a `voices` dict on the pipeline
                self.voices = sorted(list(getattr(self.pipeline, "voices", {}).keys()))
            except Exception:
                self.voices = []
            if not self.voices:
                # Fallback: hardcode the standard Kokoro voice IDs
                self.voices = [
            ...
            self.ready = True
            logger.info(f"Kokoro ready — {len(self.voices)} voices available")
        except Exception as e:
            logger.error(f"Failed to load Kokoro: {e}")
            raise

    def synthesize(self, req: SpeechRequest) -> Dict[str, Any]:
        """Synthesize a single utterance. Returns the canonical response shape."""
        self.load()
        import numpy as np
        ...
        if voice not in self.voices:
            raise ValueError(f"Unknown voice: '{voice}'. "
                             f"Available: {', '.join(self.voices[:8])}...")

        t0 = time.time()
        source_sha = hashlib.sha256(req.text.encode("utf-8")).hexdigest()

        # Kokoro's __call__ returns a generator yielding (graphemes, phonemes, audio)
        # tuples — one per chunk (split on punctuation). Concatenate audio.
        chunks_audio = []
        for _, _, audio in self.pipeline(req.text, voice=voice, speed=speed):
            chunks_audio.append(audio if hasattr(audio, "shape") else np.asarray(audio))

        if not chunks_audio:
            raise RuntimeError("Kokoro returned no audio chunks")

        full_audio = np.concatenate(chunks_audio) if len(chunks_audio) > 1 else chunks_audio[0]
        audio_seconds = float(len(full_audio)) / SAMPLE_RATE

        encoded = encode_audio(full_audio, SAMPLE_RATE, fmt)
    ...
    async def synthesize_stream(self, req: SpeechRequest) -> AsyncIterator[str]:
        """Streaming variant: yield SSE chunks as Kokoro produces them."""
        self.load()
        import numpy as np
        ...
        # Run Kokoro's generator in a thread; emit chunks via asyncio.Queue.
        q: "asyncio.Queue[Any]" = asyncio.Queue()
        DONE = {"__done__": True}

        def worker():
            try:
                for graphemes, phonemes, audio in self.pipeline(req.text, voice=voice, speed=speed):
                    arr = audio if hasattr(audio, "shape") else np.asarray(audio)
                    asyncio.run_coroutine_threadsafe(q.put({
                        "event":         "chunk",
                        "graphemes":     graphemes,
                        "audioBase64":   base64.b64encode(
                                            encode_audio(arr, SAMPLE_RATE, fmt)
                                          ).decode("ascii"),
                        "durationSec":   round(float(len(arr)) / SAMPLE_RATE, 3),
                    }), loop)
        ...
        while True:
            ...
            if chunk.get("event") == "chunk":
                total_chars += len(chunk.get("graphemes") or "")
                total_audio += chunk.get("durationSec", 0.0)
        ...
        final = {
            ...
            "model":         MODEL_NAME,
            "voice":         voice,
            "audioSeconds":  round(total_audio, 3),
            "characters":    total_chars,
            "elapsedMs":     elapsed_ms,
            "realTimeFactor": round(total_audio / max(elapsed_ms / 1000.0, 0.001), 2),
            "sourceSha256":  source_sha,
        }
    ...
    try:
        if req.stream:
            ...
            return StreamingResponse(
                kokoro_runner.synthesize_stream(req),
                media_type="text/event-stream",
                headers=safebox_headers(request_id, MODEL_NAME),
            )

        result = await asyncio.to_thread(kokoro_runner.synthesize, req)
        request_count            += 1
...
async def v1_voices():
    return {
        "model":  MODEL_NAME,
        "voices": kokoro_runner.voices if kokoro_runner.ready else [],
        "default": DEFAULT_VOICE,
        "voiceCount": len(kokoro_runner.voices) if kokoro_runner.ready else 0,
    }
...
async def capabilities():
    ...
    return {
        ...
        "transport":  transport,
        "models": {
            "loaded":     [MODEL_NAME] if (kokoro_runner.ready and MODEL_NAME) else [],
            "loading":    [MODEL_NAME] if (not kokoro_runner.ready and MODEL_NAME) else [],
            "underlying": "kokoro",
        },
        "capabilities": {
            "speech":      CAP_SPEECH,
        ...
        "health": "healthy" if kokoro_runner.ready else "loading",
    }
...
async def capacity():
    return {
        "canAccept":             kokoro_runner.ready and in_flight < MAX_QUEUE_DEPTH,
        "inFlight":              in_flight,
...
async def health():
    return Response(
        content=json.dumps({
            "status":                "healthy" if kokoro_runner.ready else "loading",
            "modelReady":            kokoro_runner.ready,
            "requestCount":          request_count,
            "inFlight":              in_flight,
            "totalCharsSynthesized": total_chars_synthesized,
            "totalAudioSeconds":     round(total_audio_seconds, 3),
        }),
        status_code=200 if kokoro_runner.ready else 503,
...
async def on_startup():
    logger.info(f"Kokoro runner starting — model={MODEL_NAME!r} "
                f"lang_code={KOKORO_LANG_CODE} default_voice={DEFAULT_VOICE}")
    setup_unix_socket()
    asyncio.get_event_loop().run_in_executor(None, kokoro_runner.load)
