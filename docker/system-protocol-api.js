...
async function checkDependencies() {
    try {
        ...
        for (const [pkg, spec] of Object.entries(deps)) {
            try {
                // Guard the dynamic require: pkg comes from system-registry.json.
                // That file is trusted, but a tampered registry must not be able to
                // load an arbitrary module by path traversal (e.g. "../../evil").
                // Restrict to a bare npm package name / scoped name. (V is defined
                // later in the file, so we inline the check here to avoid a TDZ ref
                // at startup when checkDependencies() first runs.)
                if (typeof pkg !== 'string'
                    || !/^[@a-zA-Z0-9._/-]{1,214}$/.test(pkg)
                    || pkg.includes('..')) {
                    log({ event: 'dependency_bad_name', package: String(pkg).slice(0, 64) });
                    continue;
                }
...
// Defense-in-depth input validation for values that get interpolated into
// `sh -c` command strings below. The caller is already UID- and HMAC-gated
// (only Safebox itself can reach this API), but a bug in Safebox's STM
// construction must not become container RCE. The validator table and the
// must() helper live in stmValidators.js so they can be unit-tested directly
// (see test/testStmValidators.js).
const { V, must, verifyCommit } = require('./stmValidators');
...
async function executeNpmUpdate(container, stm) {
    ...
    // Validate every value that reaches the shell string or a require() below.
    must(pkg, V.packageName, 'package name');
    must(version, V.version, 'version');
    must(workdir, V.absPath, 'workdir');
...
async function executeGitClone(container, stm) {
    ...
    // Validate url and dest before they hit the shell string
    must(url, V.gitUrl, 'git url');
    must(dest, V.absPath, 'dest path');
    ...
    return new Promise((resolve, reject) => {
        ...
        stream.on('end', async () => {
            ...
            if (inspectResult.ExitCode !== 0) {
                ...
                let head;
                try {
                    head = verifyCommit(output, commit, 'git-clone').head;
                } catch (e) {
                    reject(e);
                    return;
                }
    ...
    return new Promise((resolve, reject) => {
        ...
        stream.on('end', async () => {
            ...
            if (inspectResult.ExitCode !== 0) {
                ...
                let head;
                try {
                    head = verifyCommit(output, commit, 'git-pull').head;
                } catch (e) {
                    reject(e);
                    return;
                }
...
async function executeGitSubmodule(container, stm) {
    ...
    if (submodules.length > 256) {
        throw new Error('too many submodules (max 256)');
    }
    must(repo, V.absPath, 'repo path');
    // Every submodule path is interpolated into the shell string; validate each.
    for (const submodule of submodules) {
        // submodule paths are relative to the repo — allow the same charset as a
        // path segment sequence but no leading slash, no traversal.
        if (typeof submodule !== 'string' || !/^[a-zA-Z0-9._/-]{1,255}$/.test(submodule) || submodule.includes('..')) {
            throw new Error(`invalid submodule path: ${submodule}`);
        }
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        stream.on('end', async () => {
            ...
            if (inspectResult.ExitCode !== 0) {
                ...
                let head;
                try {
                    head = verifyCommit(output, commit, 'git-checkout').head;
                } catch (e) {
                    reject(e);
                    return;
                }
...
async function executeNginxConfig(container, stm) {
    ...
    // app becomes a filename and a symlink target in the shell string; domain and
    // upstreamHost land in the generated config. Validate all of them.
    must(app, V.ident, 'app name');
    must(domain, V.domain, 'domain');
    must(upstreamHost, V.domain, 'upstreamHost');
    if (!Number.isInteger(upstreamPort) || upstreamPort < 1 || upstreamPort > 65535) {
        throw new Error('upstreamPort must be an integer in 1-65535');
    }
    if (sslProvider !== 'letsencrypt' && sslProvider !== 'cloudflare') {
...
async function executeNginxCert(container, stm) {
    ...
    if (provider === 'letsencrypt') {
        ...
        must(certPath, V.absPath, 'certPath');
        must(keyPath, V.absPath, 'keyPath');
...
async function executeZfsSnapshot(container, stm) {
    ...
    // These reach the `zfs` binary as argv (no shell), but a malformed dataset
    // name could still hit an unintended dataset. Validate shape; mirror the
    // allowlist in opsSystem.js. Dataset must live under the safebox pool.
    must(dataset, V.zfsDataset, 'dataset');
    must(snapshot.replace(/^@/, ''), V.zfsSnapName, 'snapshot name');
...
async function executeZfsRollback(container, stm) {
    ...
    // rollback -r is destructive (discards dependent clones). Validate hard.
    must(dataset, V.zfsDataset, 'dataset');
    must(snapshot.replace(/^@/, ''), V.zfsSnapName, 'snapshot name');
...
const server = http.createServer(async (req, res) => {
    ...
    // Read body (capped to prevent memory exhaustion from a hostile/buggy peer)
    const MAX_BODY_BYTES = 1024 * 1024; // 1 MiB — STMs are small JSON control messages
    let body = '';
    let bodyBytes = 0;
    let responded = false;
    req.on('data', chunk => {
        ...
        if (bodyBytes > MAX_BODY_BYTES) {
            // Respond and tear down NOW. After req.destroy() the 'end' event will
            // not fire, so we must not defer the response to the end handler.
            responded = true;
            log({ event: 'body_too_large', peerUid, bytes: bodyBytes });
            if (!res.headersSent) {
