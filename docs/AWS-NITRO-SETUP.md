...
# Preflight, then install all components in dependency order.
# (Do NOT use the old scripts/install.sh — it installs only the system-protocol-api
# broker and skips base/system/dnsclient/autohost. The full sequence below is
# what provision-safebox-ami.sh runs.)
cd /opt/safebox-infrastructure
sudo bash scripts/preflight.sh --strict
sudo bash aws/scripts/components/base/install-base.sh       aws/scripts/components/base
sudo bash aws/scripts/components/system/install-system.sh   aws/scripts/components/system
sudo bash aws/scripts/components/dnsclient/install-dnsclient.sh aws/scripts/components/dnsclient
sudo bash aws/scripts/components/autohost/install-autohost.sh   aws/scripts/components/autohost
# Broker host-side setup (safebox-api user, HMAC key, M-of-N registry) for the
# system-protocol-api container. install-system-api-host.sh does ONLY this, despite its
# generic name — it does not install the components above.
